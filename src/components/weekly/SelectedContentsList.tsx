                ...
                actions={[
                    <Space key='actions' size='small'>
                        ...
                        <Popconfirm
                            ...
                            okText='确定'
                            cancelText='取消'>
                            <Button size='small' danger icon={<DeleteOutlined />} title='移除' />
                        </Popconfirm>
                    </Space>
                ]}>
                    ...
                    description={
                        <div>
                            ...
                            <Space wrap style={{marginTop: '4px'}}>
                                {content.section && <Tag color='purple'>{content.section}</Tag>}
                                {content.category && <Tag color='green'>{content.category.name}</Tag>}
                                {content.tags.slice(0, 2).map((tag) => (
                                    <Tag key={tag.id}>{tag.name}</Tag>
                                ))}
                                {content.tags.length > 2 && <Tag>+{content.tags.length - 2}</Tag>}
                            </Space>
                        </div>
                    }
                />
...
const SelectedContentsList: React.FC<SelectedContentsListProps> = ({contents, onRemoveContent, onReorderContents}) => {
    ...
    if (contents.length === 0) {
        return <Empty description='暂未选择内容' style={{marginTop: '20px'}} />;
    }
