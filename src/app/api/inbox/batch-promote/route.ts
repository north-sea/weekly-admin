...
import { InboxBatchPromoteSchema } from '@/lib/validations/inbox';
import { InboxService } from '@/lib/services/inbox';

// POST /api/inbox/batch-promote - 批量晋升
export async function POST(request: NextRequest) {
  try {
    ...
    const validated = InboxBatchPromoteSchema.parse(body);
    const result = await InboxService.batchPromote(validated);
    return createNextSuccessResponse(result);
  } catch (error) {
    console.error('批量晋升收件箱失败:', error);
    ...
    return createNextErrorResponse('INBOX_BATCH_PROMOTE_ERROR', '批量晋升收件箱失败', 500);
  }
}
