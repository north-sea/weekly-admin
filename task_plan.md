...
### Phase 1: 数据库适配和 AI 基础设施 [pending]
**目标**: 扩展数据库支持 AI 功能，搭建 AI 服务层

**任务**:
- [ ] 扩展 `scripts/migrate-db.ts`（以现有模式 idempotent 添加字段）
- [ ] 更新 `prisma/schema.prisma`（`contents` 模型新增字段映射）
- [ ] 扩展 contents 表（original_score, summary_score, ai_metadata）
- [ ] 扩展 contents 表（image_source, image_width, image_height）
- [ ] 创建 AI 服务层 (`src/lib/ai/`)
- [ ] 配置 Anthropic API 客户端（server-only + 环境变量）
...
**验收标准**:
- [ ] Prisma schema 包含新字段
- [ ] `pnpm db:migrate` 可重复执行且成功
- [ ] AI 客户端可以成功调用 Claude API
- [ ] TypeScript 类型无错误
