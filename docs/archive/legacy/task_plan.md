...
- [ ] AI 客户端可以成功调用已配置的模型 API（OpenAI/兼容 或 Anthropic）（待在本地用实际环境变量验证）
- [ ] TypeScript 类型无错误（仓库现有 TS 报错需单独清理）

**预计时间**: 3 小时

---

### Phase 2: AI 内容评分功能 [completed]
**目标**: 实现原文和摘要的 AI 评分
...
### Phase 3: AI 摘要生成功能 [completed]
**目标**: 实现 AI 自动生成和优化摘要

**任务**:
- [x] 实现摘要生成服务
- [x] 创建摘要生成 API 路由
- [x] 在内容编辑页添加"AI 生成摘要"按钮
- [x] 在内容编辑页添加"AI 优化摘要"按钮
- [x] 添加摘要质量评分显示（复用 summary_score）
- [x] 支持摘要重新生成
...
### Phase 5: RSS 数据源集成 [in_progress]
**目标**: 实现 RSS 源管理和自动抓取，支持去重和聚合内容处理

**任务**:
- [x] 创建 RSS 源数据模型（`scripts/migrate-db.ts` + `prisma/schema.prisma`）
- [x] 明确 RSS 源配置存储（优先 DB：`type` + `config` JSON；避免依赖 YAML 文件）
- [x] 实现 RSS 抓取服务
- [x] 实现图片提取服务（RSS 内容图 + OG/Twitter）
- [x] 实现去重服务（**关键**）
  - [x] 检查 drafts 表（Karakeep 同步）
  - [x] 检查 contents 表（已发布内容）
  - [x] URL 规范化（移除 www、跟踪参数等）
  - [x] 批量去重查询优化
  - [x] 可选：标题相似度检查（默认关闭）
- [x] 输出去重报告（用于 UI 展示：drafts/contents/相似度）
- [x] 实现聚合内容处理（**关键**）
  - [x] 聚合源检测（配置驱动 + 智能识别）
  - [x] 子链接提取
  - [x] 子链接过滤和去重
  - [x] 为每个子链接创建独立 content
- [ ] 可选：保留聚合关系（复用 `content_relations`；优先使用已有 `parent_child` 类型，避免改枚举）
- [x] 创建 RSS 源管理页面
- [x] 创建 RSS 抓取 API 路由
- [x] 创建聚合预览 API（`/api/rss/preview-aggregator`）
- [ ] 添加定时任务支持
...
**验收标准**:
- [x] 可以添加/删除/启用/停用 RSS 源（编辑 name/feed_url/config 待补）
- [x] 可以配置聚合源（type: aggregator）
- [x] 可以手动触发抓取
- [x] 抓取的内容自动写入 contents 表
- [x] 自动提取图片（OG → Twitter → 原文）（功能已实现，UI 默认未开启）
- [x] **与 Karakeep（drafts 表）去重** ⭐
- [x] **与已发布内容（contents 表）去重** ⭐
- [x] **聚合内容自动提取子链接** ⭐
- [x] **子链接也进行去重检查** ⭐
- [x] 显示去重报告（新增/重复/来源；包含 duplicates 详情）
- [x] 支持聚合条目“预览提取 + 去重标注”
